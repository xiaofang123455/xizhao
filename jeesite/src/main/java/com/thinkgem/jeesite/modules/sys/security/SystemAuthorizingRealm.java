...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if (user != null || StringUtils.isNoneBlank(token.getCaptcha())) {
			if (Global.NO.equals(user.getLoginFlag())){
				throw new AuthenticationException("msg:该已帐号禁止登录.");
			}
			byte[] salt = Encodes.decodeHex(user.getPassword().substring(0,16));
			Principal principal = new Principal(user, token.isMobileLogin());
			if (StringUtils.isNoneBlank(token.getEncryptedNo())) {
				principal.setEncryptedNo(token.getEncryptedNo());
			}
...
//		}
	...
	public static class Principal implements Serializable {
		...
		private String encryptedNo; //被加密后的工号
...
//		}
		...
		public String getEncryptedNo() {
		
			return encryptedNo;
		}

		public void setEncryptedNo(String encryptedNo) {
		
			this.encryptedNo = encryptedNo;
		}
