...
<body>
	<div class="wrapper wrapper-content">
		...
		<div class="row-fluid span8">
			<span class="span4">角色类型: <c:if test="${role.roleType=='assignment'}" >任务分配</c:if ><c:if  test="${role.roleType=='security-role'}" >管理角色</c:if ><c:if  test="${role.roleType=='user'}" >普通角色</c:if ></span>
	...
	<div class="breadcrumb">
		<form id="jumpToRoleAssign" action="${ctx}/sys/role/jumpToRoleAssign" method="post" class="hide">
			<input type="hidden" name="id" value="${role.id}"/>
		</form>
		<button id="assignButton" class="btn btn-outline btn-primary btn-sm" type="submit" title="添加人员"/>
